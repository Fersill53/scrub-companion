...
function pareTimeToMinutes(hhmm: string): number {
...
function loadSettings(): ScheduleSettings {
    ...
    if (raw) {
        try {
            ...
            if (parsed?.shiftTemplate?.startTime) return parsed;
        } catch {}
    }
}

// Sensible defaults (edit in UI)
...
export class ScheduleStore {
    ...
    upsertManualEvent(e: Omit<ScheduleEvent, 'id'> & { id?: string }): ScheduleEvent {
        ...
        const saved: ScheduleEvent = { ...e, id: e.id || safeUuid(), source: e.source ?? 'manual' };
    ...
    getEventsInRange(from: Date, to: Date): ScheduleEvent[] {
        ...
        // Manual events can override by existing type/filter/time; For now we just merge and sort
        // Later will add true override rules (maybe if I want).
    ...
    getNext(type: ScheduleEventType): ScheduleEvent | null {
        ...
        to.setDate(to.getDate() + settings.generateDaysAhead);

        const events = this.getEventsInRange(today, to).filter(e => e.type === type);
    ...
    private generateCalls(settings: ScheduleSettings, from: Date, to: Date): ScheduleEvent[] {
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            const endDt = setTimeOnDate(d, '07:00');
    ...
    private generatePaydays(settings: ScheduleSettings, from: Date, to: Date): ScheduleEvent[] {
        ...
        // Find first payday on / after start
        ...
        for (let d = new Date(first); d <= end; d.setDate(d.getDate() + cadence)) {
            const paydayAt = setTimeOnDate(d, '08:00'); // Visible in morning, easy to adjust later
            out.push({
