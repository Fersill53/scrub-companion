  ...
  },
  {
    ...
    phases: [
      ...
      },
      {
        name: 'Turnover / Wrap',
        goal: 'Document “what worked” for next time.',
        steps: [{ role: 'Scrub', priority: 'Normal', text: 'Note implant sizes used + any surgeon quirks.', cue: '' }],
      },
    ],
    pearls: 'Keep a “bailout” mindset: extra suction, cement timing, closure materials staged early.',
  },
...
export class PlaybooksComponent {
  ...
  readonly form = this.fb.group({
    ...
    phases: this.fb.array([] as FormGroup[]),

    pearls: this.fb.nonNullable.control(''),
  });

  get phasesFA(): FormArray<FormGroup> {
    return this.form.controls['phases'] as unknown as FormArray<FormGroup>;
  }
  ...
  stepsFA(phaseIndex: number): FormArray<FormGroup> {
    const phase = this.phasesFA.at(phaseIndex) as FormGroup;
    return phase.controls['steps'] as unknown as FormArray<FormGroup>;
  }
  ...
  private makeStepFG(s: PlaybookStep): FormGroup {
  ...
  private makePhaseFG(p: PlaybookPhase): FormGroup {
    return this.fb.group({
      name: this.fb.nonNullable.control(p.name ?? '', [Validators.required]),
      goal: this.fb.nonNullable.control(p.goal ?? ''),
      steps: this.fb.array((p.steps ?? []).map(s => this.makeStepFG(s))),
    });
  }
  ...
  private readPhases(): PlaybookPhase[] {
    return this.phasesFA.controls.map(phaseCtrl => {
      const p = phaseCtrl.getRawValue() as unknown as {
        name: string;
        goal: string;
        steps: Array<{ text: string; cue: string; role: StepRole; priority: StepPriority }>;
      };
